...
export const loadSpectrum = function (match, randId) {
    ...
    formatted_data.sequence1 = match.matchedPeptides[0].base_seq;
    formatted_data.linkPos1 = match.linkPos1 - 1;
    if (match.matchedPeptides[1]) {
        formatted_data.sequence2 = match.matchedPeptides[1].base_seq;
    ...
    d3.json("./get_peaklist?id=" + match.spectrumId, function (error, json) {
        if (error) {
